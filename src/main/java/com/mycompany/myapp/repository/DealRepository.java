...
public interface DealRepository extends JpaRepository<Deal, Long> {
    ...
    @Query(value = "select distinct deal from Deal deal left join fetch deal.goods",
        countQuery = "select count(distinct deal) from Deal deal")
    Page<Deal> findAllWithEagerRelationships(Pageable pageable);

    @Query("select distinct deal from Deal deal left join fetch deal.goods")
    List<Deal> findAllWithEagerRelationships();

    @Query("select deal from Deal deal left join fetch deal.goods where deal.id =:id")
